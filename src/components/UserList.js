            ...
            <div className='card-body' style={{padding: '0px'}}>
                <ul className='list-unstyled mb-0' style={{height: '630px', overflowY: 'scroll'}}>
                    {userList.map((user, index) => (
                        <li key={index}
                            className={`p-2 border-bottom ${user.name === selectedUser ? 'selected-user' : ''}`}
                            onClick={() => handleUserItemClick(user.name, user.type)} style={{cursor: 'pointer'}}>
                            <a className='d-flex justify-content-between'>
                                ...
                                <div className='pt-1'>
                                    <p className='small text-muted mb-1' key={index}> {formatDate(user.actionTime)}</p>
                                </div>
                            </a>
                        </li>
                    ))}
